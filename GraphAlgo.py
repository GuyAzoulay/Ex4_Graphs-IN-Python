...
class GraphAlgo(GraphAlgoInterface):
    ...
    def load_from_json(self, file_name: str) -> bool:
        try:
            ...
            for node in graph_Loaded['Nodes']:
                if 'pos' in graph_Loaded['Nodes'][0] and node["pos"] != None:
                    self.graph.nodes[node['id']] = tuple((float(n) for n in node["pos"].split(',')))
                else:
                    self.graph.nodes[node['id']] = tuple([np.random.uniform(0,40),np.random.uniform(0,40) ,0.0])
            # else:
            #     for node in graph_Loaded['Nodes']:
    ...
    def TSP(self, node_lst: list[int]) -> (list[int], float):
        ...
        for list1 in all_Paths:
            ...
            res = any(t in node_lst for t in list1)
            if res:
    ...
    def plot_graph(self) -> None:
        ...
        while running:
            ...
            for e in self.graph.edges.keys():
                ...
                draw_arrow("black", [src_x, src_y], [(src_x + dest_x) / 2, (src_y + dest_y) / 2],screen)
...
if __name__ == '__main__':
    ...
    # tt= time.time()
    # print("TSP",tt-gg)
    #print(galgo.shortest_path(0,524))
    #print(galgo.TSP([0,7,8]))
    #print(galgo.TSP([0,7,8]))
    # galgo.save_to_json("A5.json")
    # for x in a:
    #     print(x)
